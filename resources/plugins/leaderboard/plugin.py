...
#   leaderboard:
#     podName: "a-pod-name"
#     addAsCaddyWebsite: true/false
#
# "podName" and "addAsCaddyWebsite" are essentially dictionary keys, and it helps to keep those keys in an
# enum in order to prevent typos.
class PluginContent(Enum):
    POD_NAME = "podName"
    ADD_AS_CADDY_WEBSITE = "addAsCaddyWebsite"
...
@click.pass_context
def leaderboard(ctx):
    """Commands for the Leaderboard plugin"""
...
def _entrypoint(ctx, plugin_content: dict, warnet_content: dict):
    ...
    match hook_value:
        case (
            ...
            if data:
                _launch_pod(ctx, install_name=hook_value.value.lower() + "-leaderboard", **data)
            else:
                _launch_pod(ctx, install_name=hook_value.value.lower() + "-leaderboard")
        case HookValue.PRE_NODE:
            name = warnet_content[PLUGIN_ANNEX][AnnexMember.NODE_NAME.value] + "-pre-leaderboard-pod"
            _launch_pod(ctx, install_name=hook_value.value.lower() + "-" + name, podName=name)
        case HookValue.POST_NODE:
            name = warnet_content[PLUGIN_ANNEX][AnnexMember.NODE_NAME.value] + "-post-leaderboard-pod"
            _launch_pod(ctx, install_name=hook_value.value.lower() + "-" + name, podName=name)
...
def get_data(plugin_content: dict) -> Optional[dict]:
    data = {
        ...
        for key in (PluginContent.POD_NAME.value, PluginContent.ADD_AS_CADDY_WEBSITE.value)
...
def _launch_pod(
    ctx, install_name: str = "leaderboard", podName: str = "leaderboard-pod", addAsCaddyWebsite: bool = False
):
    if addAsCaddyWebsite:
        command = (
            f"helm upgrade --install {install_name} {ctx.obj[PLUGIN_DIR_TAG]}/charts/leaderboard "
            f"--set podName={podName} --set addAsCaddyWebsite=true"
        )
        log.info(command)
